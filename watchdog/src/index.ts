...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info(`WatchDog running (trigger: @${ASSISTANT_NAME})`);
...
async function main(): Promise<void> {
  ...
  loadState();
  const expiredSessions = deleteOldSessions(30);
  if (expiredSessions > 0) logger.info({ expired: expiredSessions }, 'Pruned old sessions');
  restoreRemoteControl();
...
if (isDirectRun) {
  main().catch((err) => {
    logger.error({ err }, 'Failed to start WatchDog');
    process.exit(1);
  });
}
