...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // In Docker-in-Docker mode we skip this: projectRoot is inside the watchdog
  ...
  // Always write settings.json so env vars (especially ANTHROPIC_API_MODEL) stay current.
...
function buildContainerArgs(
  ...
  // Direct credential injection — no proxy
  ...
  if (anthropicBaseUrl) {
    ...
    args.push('-e', `ANTHROPIC_API_URL=${anthropicBaseUrl}`);
  }
  ...
  // URL used by watchdog itself (CLAUDE_REMOTE_API_URL).
  const apiUrl =
...
export async function runContainerAgent(
  ...
  const containerName = `watchdog-${safeName}-${Date.now()}`;
