...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'WatchDog';
...
Messages must start with the trigger pattern (default: `@WatchDog`):
- `@WatchDog what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @WatchDog` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @WatchDog what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@WatchDog what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@WatchDog add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@WatchDog remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@WatchDog list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@WatchDog remember I prefer dark mode` | Add to global memory |
...
```
User: @WatchDog remind me every Monday at 9am to review the weekly metrics
...
```
User: @WatchDog at 5pm today, send me a summary of today's emails
...
From any group:
- `@WatchDog list my scheduled tasks` - View tasks for this group
- `@WatchDog pause task [id]` - Pause a task
- `@WatchDog resume task [id]` - Resume a paused task
- `@WatchDog cancel task [id]` - Delete a task

From main channel:
- `@WatchDog list all tasks` - View tasks from all groups
- `@WatchDog schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@WatchDog add group "Name"` in main |
