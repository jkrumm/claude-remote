...
function createDockerRoutes(proxyUrl: string, tag: string): Elysia {
  ...
  return new Elysia()
    ...
    // GET /containers — all containers with health, uptime, restart count
    .get(
      '/containers',
      async () => {
    ...
    // GET /stats — CPU + memory + network for all running containers
    .get(
      '/stats',
      async () => {
    ...
    // GET /logs/:name?tail=100 — last N log lines for a container
    .get(
      '/logs/:name',
      async ({ params, query }) => {
    ...
    // GET /summary — single high-level overview for AI context
    .get(
      '/summary',
      async () => {
...
export const dockerHomelabRoutes = new Elysia({ prefix: '/docker/homelab' }).use(
  createDockerRoutes(
    process.env.DOCKER_PROXY_URL_HOMELAB ?? 'http://docker-proxy:2375',
    'Docker - HomeLab',
  ),
)
...
export const dockerVpsRoutes = new Elysia({ prefix: '/docker/vps' }).use(
  createDockerRoutes(
    process.env.DOCKER_PROXY_URL_VPS ?? '',
    'Docker - VPS',
  ),
)
