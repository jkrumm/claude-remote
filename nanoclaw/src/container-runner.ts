...
function buildVolumeMounts(
  ...
  if (isMain) {
    // Main gets the project root read-only so the agent can read the codebase.
    // In Docker-in-Docker mode we skip this: projectRoot is inside the nanoclaw
    // container image, not a host filesystem path the Docker daemon can mount.
    if (!IN_DOCKER_MODE) {
      mounts.push({
    ...
    mounts.push({
      hostPath: toHostMountPath(groupDir),
    ...
    if (fs.existsSync(globalDir)) {
      mounts.push({
        hostPath: toHostMountPath(globalDir),
  ...
  // Always write settings.json so env vars (especially ANTHROPIC_API_MODEL) stay
  // current. Agent teams subagents inherit these env vars — without ANTHROPIC_API_MODEL
  // here they fall back to Claude Code's default model (sonnet) which may not be
  // available on custom API endpoints.
  ...
  if (model) settingsEnv['ANTHROPIC_API_MODEL'] = model;
  fs.writeFileSync(
    settingsFile,
  ...
  mounts.push({
    hostPath: toHostMountPath(groupSessionsDir),
  ...
  mounts.push({
    hostPath: toHostMountPath(groupIpcDir),
  ...
  mounts.push({
    hostPath: toHostMountPath(groupAgentRunnerDir),
